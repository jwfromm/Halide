...
class BinGemm2A1B {
    ...
    Target t;
public:
    BinGemm2A1B(int m, int n, int p, bool gpu = false) :
    A(Buffer<float>(p,m)),
    ...
    t(get_host_target())
    {
        ...
        binarize_A1(x, y) = sum(select(At(64*x + r, y) > 0, cast<int64_t>(1) << r.x, cast<int64_t>(0)), "compress_A1"); 
    ...
    void realize(float *A_array, int64_t *B_array, float *out_array) {
