...
class BinConvolve {
    ...
    Target t;
public:
    BinConvolve(int w, int h, int channels, int filters, int size, int stride, bool pad, bool gpu = false) :
    ...
    t(get_host_target())
    {
        ...
        if (!gpu) {
            ...
            Var block, thread;
            binarize_weights.compute_root();
            binarize_weights.gpu_blocks(f).gpu_threads(x);
            output.reorder(y, x);
            binarize_input.compute_at(output, x);
            bit_mask.compute_at(output, x);
    ...
    void realize(float *in_array, float *weight_array, float *out_array) {
